...
try {
  ...
  if (has('dry')) {
    ...
    /**
     * ALSO EMIT AN ESM MODULE, because lib/tx/coverage.js has to import this and
     * a .json import does not work in both places this codebase runs code.
     * Next's bundler resolves `import x from './f.json'`; bare Node — which is
     * how every scripts/verify-*.mjs loads application modules, through
     * scripts/resolve-extensionless.mjs — refuses it without an import
     * attribute Next 14 does not reliably emit. A module that imports fine in
     * the app and throws in the verifier is a guard that cannot run.
     *
     * Generated, never hand-edited. Same contract as the JSON.
     */
    writeFileSync(
      OUT.replace(/\.json$/, '.js'),
