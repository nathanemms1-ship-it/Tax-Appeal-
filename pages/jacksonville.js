...
{(() => {
  ...
  if (today >= preOrderOpen && today < windowOpen) {
    ...
    return (
      <div style={barStyle}>
        🔒 Reserve your Duval County spot now — TRIM notices start arriving in {days} days. Lock in the $89 rate today; we file the moment your county's window opens. <a href="/apply" style={{ color: '#0F1F3D', textDecoration: 'underline', marginLeft: 6, fontWeight: 700 }}>Get started →</a>
      </div>
    );
  }
...
<section style={{background:C.navy,padding:"64px 40px",color:C.white}}><div style={{maxWidth:900,margin:"0 auto"}}><div style={{fontSize:12,color:C.gold,textTransform:"uppercase",letterSpacing:"2px",marginBottom:16}}>Jacksonville, Florida — Property Tax Appeal Service</div><h1 className="ht" style={{fontFamily:"'DM Serif Display',serif",fontSize:42,lineHeight:1.15,marginBottom:16}}>Jacksonville Property Tax Appeal — $89 Flat Fee</h1><p style={{fontSize:18,color:"#8596AF",lineHeight:1.6,maxWidth:640,marginBottom:32}}>Jacksonville homeowners: challenge your Duval County VAB assessment before the 25-day TRIM notice deadline. No two-way review risk. TaxAppeal files for $89 flat.</p><div className="hs" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:16,marginBottom:32}}>{[["25 days","From TRIM notice"],["$50","VAB petition fee"],["$89","TaxAppeal fee"],["Duval Co.","Service area"]].map(([n,l])=>(<div key={l} style={{background:"#0F1F3D",borderRadius:10,padding:"16px",textAlign:"center"}}><div style={{fontFamily:"'DM Serif Display',serif",fontSize:n.length>8?14:28,color:C.gold}}>{n}</div><div style={{fontSize:11,color:"#5A7A9F",marginTop:4}}>{l}</div></div>))}</div><button className="btn-p" style={{background:C.gold,color:C.darkNavy,fontSize:17,padding:"18px 44px"}} onClick={go}>File My Jacksonville Appeal — $89 →</button><div style={{fontSize:13,color:"#5A7A9F",marginTop:12}}>Cited under Florida Statute §194.011. No two-way review risk.</div></div></section>
<section style={{padding:"56px 40px",background:C.white}}><div style={{maxWidth:800,margin:"0 auto"}}><h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:28,textAlign:"center",marginBottom:32}}>Why Jacksonville Homeowners Should Appeal</h2><div className="g2" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>{[["Receipt Deadline — Not Postmark","Florida requires physical VAB receipt within 25 days of your TRIM notice. TaxAppeal mails your petition 7–10 days early to ensure timely receipt."],["No Two-Way Risk","Florida's VAB cannot raise your assessment. The worst outcome is your value stays the same."],["Post-2020 Appreciation","Jacksonville's rapid appreciation created widespread over-assessment as models overshoot corrections."],["Navy Market Distortions","NAS Jacksonville and Mayport Naval Station PCS sales distort civilian comparable data. TaxAppeal focuses on arm's-length transactions."]].map(([t,d])=>(<div key={t} style={{background:C.bg,borderRadius:12,padding:24}}><div style={{fontFamily:"'DM Serif Display',serif",fontSize:16,color:C.darkNavy,marginBottom:8}}>{t}</div><p style={{fontSize:13,color:C.bodyGray,lineHeight:1.65}}>{d}</p></div>))}</div></div></section>
<section style={{padding:"56px 40px",background:C.bg}}><div style={{maxWidth:720,margin:"0 auto"}}><h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:28,textAlign:"center",marginBottom:32}}>Jacksonville Property Tax FAQ</h2>{faqs.map(([q,a],i)=>(<div key={i} style={{background:C.white,border:`1.5px solid ${openFaq===i?C.navy:C.border}`,borderRadius:10,marginBottom:10,overflow:"hidden"}}><div onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{padding:"18px 20px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",gap:12}}><span style={{fontSize:14,fontWeight:500,color:C.darkNavy}}>{q}</span><span style={{color:C.navy,fontSize:18,flexShrink:0}}>{openFaq===i?"−":"+"}</span></div>{openFaq===i&&<div style={{padding:"0 20px 18px",fontSize:14,color:C.bodyGray,lineHeight:1.7}}>{a}</div>}</div>))}</div></section>
<div style={{background:C.darkNavy,padding:"56px 40px",textAlign:"center"}}><h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:34,color:C.white,marginBottom:12}}>File Before Your TRIM Deadline</h2><p style={{color:"#A0B4CC",fontSize:16,marginBottom:32}}>$139 all-in — $89 service plus the $50 Duval County petition fee we pay for you. 25-day receipt deadline from your TRIM notice.</p><button className="btn-p" style={{background:C.gold,color:C.darkNavy,fontSize:18,padding:"18px 48px"}} onClick={go}>Start My Jacksonville Appeal — $89 →</button></div>
