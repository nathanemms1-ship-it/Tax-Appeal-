...
export default function Portal() {
  ...
  const status = order ? (STATUS_CONFIG[order.dispute_status] || STATUS_CONFIG.pending) : null;
  ...
  const timelineSteps = order ? [
    ...
    {
      label: order.dispute_status && order.dispute_status !== 'pending'
        ? 'Decision Received'
        : 'Awaiting Decision',
      sublabel: order.dispute_status && order.dispute_status !== 'pending'
        ? `County ruled: ${STATUS_CONFIG[order.dispute_status]?.label}`
        : 'Typical wait: 4–12 weeks',
      done: order.dispute_status && order.dispute_status !== 'pending',
      ...
      <div style={styles.page}>
        <header style={styles.header}>
          <a href="/" style={styles.logo}>
            <span style={styles.logoAccent}>Tax</span>Appeal USA
          </a>
          {view === 'dashboard' && (
        ...
        <main style={{ maxWidth: 640, margin: '0 auto', padding: '52px 20px 80px' }}>
          ...
          {view === 'login' && (
            <>
              <div style={{ marginBottom: 36 }}>
                <h1 style={{ color: '#fff', fontSize: 30, fontWeight: 700, margin: '0 0 8px', letterSpacing: '-0.5px' }}>
                  My Appeal Portal
                </h1>
                <p style={{ color: '#475569', margin: 0, fontSize: 16 }}>
                  Track the status of your property tax dispute.
                </p>
              </div>
              ...
              <p style={{ color: '#334155', fontSize: 13, textAlign: 'center', margin: '16px 0 0' }}>
                Use the email & password you created when you filed your dispute.
                <br />
                Need help? <a href="mailto:support@taxappealusa.com" style={{ color: '#22c55e', textDecoration: 'none' }}>support@taxappealusa.com</a>
              </p>
            </>
          )}
          ...
          {view === 'dashboard' && order && (
            <>
              <div style={{ marginBottom: 36 }}>
                <p style={{ color: '#22c55e', fontSize: 13, fontWeight: 600, margin: '0 0 6px', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                  Appeal Portal
                </p>
                <h1 style={{ color: '#fff', fontSize: 28, fontWeight: 700, margin: 0, letterSpacing: '-0.4px' }}>
                  {order.name ? `Welcome, ${order.name.split(' ')[0]}` : 'Your Dispute'}
                </h1>
              </div>
              ...
              <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6, flexWrap: 'wrap' }}>
                    ...
                    {order.decision_date && (
                      <span style={{ color: '#475569', fontSize: 13 }}>
                        {formatDate(order.decision_date)}
                      </span>
                    )}
                  </div>
                ...
                {order.savings_amount > 0 && (
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div style={{ color: '#64748b', fontSize: 11, fontWeight: 600, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 4 }}>Savings</div>
                    <div style={{ color: '#22c55e', fontSize: 26, fontWeight: 700, lineHeight: 1 }}>
                      ${order.savings_amount.toLocaleString()}
                    </div>
                  ...
                  <div key={i} style={{ display: 'flex', gap: 16, marginBottom: i < timelineSteps.length - 1 ? 28 : 0 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0 }}>
                      <div style={{
                        ...
                        background: step.done ? '#22c55e' : '#1e293b',
                        border: step.done ? '2px solid #22c55e' : '2px solid #334155',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        color: step.done ? '#fff' : '#334155',
                    ...
                    <div style={{ paddingTop: 4, paddingBottom: i < timelineSteps.length - 1 ? 8 : 0 }}>
                      <div style={{ color: step.done ? '#fff' : '#334155', fontSize: 15, fontWeight: 600, marginBottom: 3 }}>
                        {step.label}
                      </div>
              ...
              <p style={{ color: '#1e293b', fontSize: 13, textAlign: 'center', marginTop: 28 }}>
                ...
                <a href="mailto:support@taxappealusa.com" style={{ color: '#22c55e', textDecoration: 'none' }}>
