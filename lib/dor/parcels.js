 ...
 */
import {
  normalizeAddr, normSpace, anchoredPattern, rowMatches, addressVariants,
  addressVariantTiers, orIlike, stripTrailingLocality,
...
export async function suggestAddresses(query, { limit = 8, zip = null } = {}) {
  ...
  const run = (variants, withZip) => {
    let sel = db
      ...
      .or(orIlike('phy_addr1', variants.map((v) => `%${v}%`)))
      .limit(limit);
    if (withZip && zip) sel = sel.eq('phy_zipcd', String(zip).trim().slice(0, 5));
    return sel;
  };

  const fetchTier = async (variants) => {
    ...
    if (!error && zip && (!data || !data.length)) {
      ...
      ({ data, error } = await run(variants, false));
    }
    return { data: data || [], error };
  };

  /**
   * WHAT WAS TYPED IS QUERIED BEFORE WHAT WAS INFERRED. 27 Aug 2026.
   *
   * One `.or()` over both tiers meant the unit-stripped spelling filled the
   * eight slots with arbitrary units of the same building, and the unit the
   * customer was in the middle of typing never appeared. See
   * addressVariantTiers. Typing MORE of your address made the list no better,
   * which is the opposite of what an autocomplete is for — and on the one
   * property type where it matters, condos, it is the whole game.
   *
   * The broad tier still runs, because "8023 MARBELLA CREEK AVE APT 2" must
   * still find the building when the roll files no unit at all. It just runs
   * SECOND, and only to fill slots the specific tier left empty.
   */
  const tiers = addressVariantTiers(q);
  let { data, error } = await fetchTier(tiers.specific);

  if (!error && data.length < limit && tiers.broad.length) {
    const wider = await fetchTier(tiers.broad);
    if (!wider.error) {
      const seen = new Set(data.map((r) => r.parcel_id));
      data = data.concat(wider.data.filter((r) => !seen.has(r.parcel_id))).slice(0, limit);
    }
  }
...
export async function findParcel({ street, zip, city = null }) {
  ...
  /**
   * ============================================================================
   * BOTH SUFFIX SPELLINGS. MEASURED, NOT INFERRED. 7 Sept 2026.
   * ============================================================================
   * normalizeAddr applies SUFFIXES to EVERY word, and SUFFIXES rewrites 25 of
   * them — STREET, AVENUE, ROAD, DRIVE, LANE, COURT, CIRCLE, BOULEVARD, PLACE,
   * TERRACE, PARKWAY, TRAIL, HIGHWAY, SQUARE, POINT, RIDGE, COVE and the eight
   * directionals. Florida street NAMES contain these constantly. "815 MAPLE
   * RIDGE RD" becomes "815 MAPLE RDG RD", and the ILIKE runs against the roll's
   * RAW phy_addr1, so the pattern cannot match.
   *
   * The TERMINAL_SUFFIXES comment in addressMatch.js describes this hazard as
   * hypothetical. It is not. scripts/fl/lookup-probe.mjs, 300 real PHY_ADDR1
   * values fed straight back through this function:
   *
   *     15 of 300 addresses contain an interior suffix word   (5.0%)
   *     13 of those 15 do not round-trip                      (86.7%)
   *
   *     417 BOCA CIEGA POINT BLVD N      no_parcel, 0 rows
   *     815 MAPLE RIDGE RD               no_parcel, 0 rows
   *     900 COVE CAY DR # 1E             no_parcel, 0 rows
   *     17708 LONG POINT DR              no_parcel, 0 rows
   *     2020 WORLD PARKWAY BLVD # 48     no_parcel, 0 rows
   *
   * Roughly 4.3% of ALL Florida lookups fail from this one cause, and every one
   * of them tells a homeowner we have no record of their property. This is the
   * same class of defect as the ZIP hard filter, found the same way and fixed
   * only after measuring it here rather than inferring it from Texas.
   *
   * The spelled-out form is LONGER, so its `%...%` pattern is narrower, not
   * wider — this cannot pull in rows a working lookup did not already see. The
   * `exact` rule below still prefers an exact hit, and rowMatches still decides.
   */
  const spelled = normalizeAddr(street, { interiorSuffixes: false });
  const altTiers = spelled && spelled !== addr ? addressVariantTiers(spelled) : null;

  // `let`, because the locality fallback below rebuilds it and `run` closes over
  // the binding rather than the value.
  let variants = altTiers
    ? [...new Set([...addressVariants(addr), ...addressVariants(spelled)])]
    : addressVariants(addr);
  /**
   * RETRIEVAL IS TIERED; THE DECISION IS NOT. 27 Aug 2026.
   *
   * `variants` stays the full list, because rowMatches and the `exact` rule
   * below must judge a retrieved row against every spelling — narrowing the
   * DECISION would reject rows the query legitimately returned.
   *
   * `retrieval` is what goes into the SQL, and it starts at the spelling the
   * customer typed. Both tiers under one `.or()` and one `.limit(12)` is how a
   * condo unit was evicted from its own result set by 600 of its neighbours:
   * the `exact` rule twenty lines down exists precisely to pick that row, could
   * only ever pick a row that survived retrieval, and so never fired. The
   * address then returned `ambiguous` — a question the visitor cannot answer
   * from a list their unit is not on. See addressVariantTiers.
   */
  /**
   * The tiering is preserved, not collapsed: the alternate spelling's SPECIFIC
   * variants join the specific tier and its BROAD ones join the broad tier. A
   * spelled-out address is still a specific spelling of the customer's address,
   * and merging it into `broad` would have made it unreachable until the first
   * tier had already failed — which for these addresses it always does, with
   * zero rows, because that is the bug.
   */
  const baseTiers = addressVariantTiers(addr);
  const tiers = altTiers ? {
    specific: [...new Set([...baseTiers.specific, ...altTiers.specific])],
    broad: [...new Set([...baseTiers.broad, ...altTiers.broad])],
  } : baseTiers;
  ...
  const run = (useZip) => {
    let sel = db
      ...
      .select('*')
      .eq('asmnt_yr', ROLL_YEAR)
      .or(orIlike('phy_addr1', retrieval.map(anchoredPattern)))
  ...
  /**
   * WIDEN TO THE UNIT-STRIPPED SPELLING ONLY WHEN THE TYPED ONE FOUND NOTHING.
   *
   * This is the rescue the unit strip was added for — a roll that files the
   * unit in PHY_ADDR2, or not at all, so "…AVE APT 2" matches no PHY_ADDR1.
   * Gated on zero rows RETRIEVED for the same reason the locality fallback
   * below is: if the specific tier returned anything, the customer's own
   * spelling reached real candidates, and replacing them with a query that
   * returns strictly more rows is how somebody is handed a neighbour's unit.
   */
  if (!error && !(data || []).length && tiers.broad.length) {
  ...
  if (!error && !(data || []).length) {
    ...
    const trimmedSpelled = spelled ? stripTrailingLocality(spelled) : null;
    if (trimmed && trimmed !== addr) {
      // Both spellings here too, for the same reason and with the same care:
      // the locality must be stripped from BOTH, or the fallback puts the city
      // back into the pattern list it exists to remove.
      variants = (trimmedSpelled && trimmedSpelled !== trimmed)
        ? [...new Set([...addressVariants(trimmed), ...addressVariants(trimmedSpelled)])]
        : addressVariants(trimmed);
      // Nothing was retrieved by either tier, so there is no candidate set to
      // protect — retrieve on every spelling of the trimmed address at once.
      retrieval = variants;
      ({ data, error } = await run(true));
  ...
  /*
    THE COUNTY OF A NEAR MISS IS KNOWABLE. 26 Aug 2026.

    The comment above says county splitting "was never possible, because county is
    derived from a matched parcel and on a miss there is none". That holds for
    `no_parcel` -- nothing was retrieved, so there is nothing to read a county off.
    It does NOT hold for `no_parcel_near_miss`, which is DEFINED by rows having come
    back. Those rows carry co_no and are discarded on the next line.

    So the two outcomes that are our own bugs -- near miss and ambiguous -- are
    exactly the two where the roll itself can say where they happened, at no extra
    query and with no address stored. Read off the roll, not off the ZIP: coverage.js
    is explicit that a ZIP is not a jurisdiction.
  */
  const retrievedCoNos = [...new Set((data || []).map((r) => r.co_no).filter((n) => n != null))];
  data = (data || []).filter((r) => rowMatches(r.phy_addr1, variants));
  if (!data.length) return { noMatch: true, nearMisses: retrievedBeforeFilter, coNos: retrievedCoNos };
  ...
  if (data.length > 1) {
    ...
    return {
      ambiguous: true,
      // Same reasoning as the near-miss above: these ARE roll rows, so the county
      // is in hand. Diagnostic only -- nothing here reaches a petition.
      coNos: [...new Set(data.map((r) => r.co_no).filter((n) => n != null))],
...
/**
 * The county NAME shared by every row we retrieved, or null.
 *
 * NULL WHEN THEY DISAGREE, ON PURPOSE. Once a ZIP miss retries without the ZIP,
 * one street name can match rows in two Florida counties. Picking the first would
 * put a confident county on a row whose whole purpose is to be evidence, and a
 * wrong one is worse than none -- this column already has a history of that
 * (join-waitlist stored "29" for Hillsborough and everything downstream missed).
 *
 * Two counties disagreeing is itself the finding: it means the fallback pattern
 * cut too much. An empty county on a near miss says exactly that.
 */
function soleCountyName(coNos) {
  if (!Array.isArray(coNos) || coNos.length !== 1) return null;
  const no = Number(coNos[0]);
  ...
  if (!parcel || parcel.noMatch) {
    ...
    */
    if (nearMisses > 0) {
      return {
        found: false,
        reason: 'no_parcel_near_miss',
        nearMisses,
        county: soleCountyName(parcel?.coNos),
        message: missMessage,
      };
    }
    // Genuinely nothing retrieved, so genuinely no county. Left null rather than
    // guessed -- a wrong county here would be worse than an empty one.
    return { found: false, reason: 'no_parcel', nearMisses: 0, county: null, message: missMessage };
  }
  if (parcel.ambiguous) {
    return {
      found: false,
      reason: 'ambiguous',
      candidates: parcel.candidates,
      county: soleCountyName(parcel?.coNos),
      message: 'Several properties share this address. Please choose yours.',
    };
  }
