...
export async function findComps(subject, { rollYear = 2026 } = {}) {
  ...
  const { rows, level } = await fetchCandidates(db, subject, rollYear);
  ...
  const { data: parcels, error } = await db
    .from('parcels')
    .select('parcel_id, phy_addr1, phy_city, phy_zipcd, tot_lvg_area, act_yr_blt, dor_uc, no_res_unts')
    .eq('co_no', subject.co_no)
    .eq('asmnt_yr', rollYear)
    .in('parcel_id', ids);
  if (error) { console.error('[comps] parcel join failed:', error.message); return { comps: [], sufficient: false, reason: 'join_failed' }; }
  ...
  let band = null, ageBand = null, inBand = [];
  outer: for (const ab of AGE_BANDS) {
    for (const b of SIZE_BANDS) {
      const set = enriched.filter((c) => {
        ...
        if (!subjYear || !c.yearBuilt) return true;   // unknown age is not disqualifying
        return Math.abs(Number(c.yearBuilt) - subjYear) <= ab;
      });
      if (set.length >= MIN_COMPS) { band = b; ageBand = ab; inBand = set; break outer; }
    }
  }
  if (!band) {
    return {
  ...
  const supportsReduction = indicatedValue != null && subject.jv != null
