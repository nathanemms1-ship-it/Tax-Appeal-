...
export async function findComps(subject, { rollYear = 2026 } = {}) {
  ...
  // Tightest size band that yields enough comps. See the header for why this
  // matters more than it looks.
  let band = null, inBand = [];
  for (const b of SIZE_BANDS) {
    const set = enriched.filter(
      (c) => Math.abs(c.sqft - subject.tot_lvg_area) / subject.tot_lvg_area <= b
    );
    if (set.length >= MIN_COMPS) { band = b; inBand = set; break; }
  }
  if (!band) {
    return {
      comps: enriched
