...
/**
 * ============================================================================
 * `interiorSuffixes: false` — ADDED 7 Sept 2026, FOR TEXAS, ADDITIVELY.
 * ============================================================================
 *
 * The TERMINAL_SUFFIXES comment below already names this hazard exactly:
 * rewriting a suffix word wherever it appears corrupts street NAMES that
 * contain one. SUFFIXES holds COURT, COVE, POINT, NORTH, WEST and more, and it
 * is applied to every word.
 *
 * scripts/tx/lookup-probe.mjs proved it on El Paso. 200 situs addresses taken
 * from the roll and fed straight back: 9 returned no_parcel with ZERO rows
 * retrieved — addresses for houses sitting in our own database. Every one was
 * this:
 *
 *     roll spells            we searched for
 *     11137 VOYAGER COVE DR  11137 VOYAGER CV DR
 *     3128 MOON POINT PL     3128 MOON PT PL
 *     5840 SUN COURT CIR     5840 SUN CT CIR
 *     7858 NORTH LOOP DR     7858 N LOOP DR
 *     7836 WEST DR           7836 W DR
 *
 * The abbreviation is applied to the SEARCH TERM while the ILIKE runs against
 * the roll's raw text, so the pattern cannot match a district that spells the
 * name out. El Paso spells it out.
 *
 * WHY A FLAG AND NOT A FIX TO THE TABLE. Moving COVE and POINT into
 * TERMINAL_SUFFIXES would be the tidier change and it would alter matching for
 * Florida, which is live and taking money today, on the strength of a Texas
 * measurement. Florida has almost certainly been losing the same class of
 * address — "KINGS LANDING DR" is the file's own example — but that should be
 * established by running the same probe against the Florida roll, not assumed.
 * Until then this parameter is opt-in: every existing caller passes nothing and
 * gets byte-identical behaviour.
 */
export function normalizeAddr(s, { interiorSuffixes = true } = {}) {
  let out = String(s || '')
  ...
  const words = out.split(' ').map((w, i, arr) => (
    (interiorSuffixes || i === arr.length - 1) ? (SUFFIXES[w] || w) : w
  ));
...
/**
 * ONE STREET TYPE, THREE SPELLINGS, AND THE DISTRICTS DO NOT AGREE.
 *
 * Counted over the seven Texas rolls we hold, trailing token only, 9 Sept 2026:
 *
 *              TRL     TRAIL    TR        TER   TERR   TERRACE
 *   El Paso    202        19     -          8      -         3
 *   Kaufman   3090       279   151          -      -         -
 *   Nueces     770         8    99          1     53        21
 *   Tarrant      -         -  5823          -   3133         -
 *   Taylor      17        37   966          -      -        16
 *   Wichita    425         -    35          -     88        15
 *   Jefferson  253         -     -          -      -         -
 *
 * Kaufman uses ALL THREE spellings of trail in its own roll. So there is no
 * canonical form to normalise to — mapping TRAIL to TRL loses Tarrant's 5,823,
 * and mapping it to TR loses Kaufman's 3,090. rowMatches compares against the
 * roll's LITERAL text (normCompare only collapses spaces and dots), so the
 * spelling the district used has to appear among the variants or the parcel
 * cannot be found at all.
 *
 * Hence an equivalence class rather than a rewrite: emit every member, let
 * retrieval try them all, and let findParcel prefer the exact match. This is the
 * same reasoning as SAINT/ST above — "retrieval has to try both real spellings".
 *
 * Tarrant is what surfaced it (~19,600 parcels, 3.3% of the district), but
 * Kaufman's 151 TR and Nueces's 99 TR + 53 TERR were already unreachable before
 * Tarrant existed.
 */
...
/**
 * Applied to the LAST WORD ONLY, for the reason TERMINAL_SUFFIXES documents:
 * PARK is an interior name word 5,234 times in Tarrant ("ESTES PARK RD") and a
 * trailing street type once. Rewriting it anywhere it appears would corrupt
 * thousands of addresses to rescue one.
 */
function suffixClassVariants(s) {
  const parts = String(s).split(' ').filter(Boolean);
  if (parts.length < 2) return [s];
  const last = parts[parts.length - 1];
  const cls = SUFFIX_CLASSES.find((c) => c.includes(last));
  if (!cls) return [s];
  const head = parts.slice(0, -1).join(' ');
  return cls.map((alt) => `${head} ${alt}`);
}

function roadVariants(s) {
  const out = new Set(suffixClassVariants(s));
...
/**
 * Every spelling of ONE base string — the three families above, then ordinals.
 *
 * Split out of addressVariants so the unit-bearing spelling and the
 * unit-stripped one can be retrieved in separate queries. See
 * addressVariantTiers.
 */
function expandBase(addr) {
  const bases = [...new Set(
    saintVariants(String(addr)).flatMap(roadVariants).flatMap(directionalVariants)
  )];
...
/**
 * ============================================================================
 * THE UNIT-STRIPPED SPELLING EVICTED THE ONE THE CUSTOMER TYPED. 27 Aug 2026.
 * ============================================================================
 * addressVariants returns the address as typed AND with its unit removed, and
 * both went into one `.or()` under one `.limit()`. For a condo that is not a
 * wider net, it is a flood: "1750 N BAYSHORE DR 3204" also carries "1750 N
 * BAYSHORE DR", which `%...%` matches against every one of the tower's 600-odd
 * units. Postgres has no ORDER BY to obey, so it returns an arbitrary slice —
 * and the customer's own unit is very unlikely to be in it.
 *
 * The consequences ran in both directions and both were silent:
 *
 *   suggestAddresses  offered six units nobody asked for and never the one
 *                     being typed. Typing more of the address changed nothing,
 *                     because the extra characters only ever narrowed a variant
 *                     that was not the one filling the result set.
 *   findParcel        never saw the exact row, so the `exact` rule below it —
 *                     the rule whose entire job is "an exact match beats a
 *                     prefix match", and which the comment there says is what
 *                     "makes unit stripping safe" — had nothing to select. A
 *                     resolvable address returned `ambiguous` instead, and
 *                     `ambiguous` is the largest no-finding outcome on the site.
 *
 * The guard was sound and unreachable: it can only prefer an exact row that
 * survived retrieval. Correctness that depends on an unordered LIMIT is not
 * correctness.
 *
 * SO RETRIEVAL IS TIERED, AND SPECIFICITY DECIDES THE TIER. `specific` is every
 * spelling of what the customer actually typed. `broad` is what is left after
 * removing the unit. A caller queries `specific` first and only widens when it
 * comes back empty, which is the state the unit strip was added to rescue.
 *
 * `broad` is empty whenever stripUnit changed nothing, so a plain house address
 * runs exactly one query and behaves as it did before.
 */
export function addressVariantTiers(addr) {
  const s = String(addr);
  const parts = s.split(' ').filter(Boolean);
  if (parts.length < 2) return { specific: [s], broad: [] };

  const specific = expandBase(s);
   ...
   */
  const strippedBase = stripUnit(s);
  if (strippedBase === s) return { specific, broad: [] };

  const seen = new Set(specific);
  return { specific, broad: expandBase(strippedBase).filter((v) => !seen.has(v)) };
}

/**
 * Every spelling, both tiers, specific first.
 *
 * Kept because rowMatches is a DECISION over the whole set and must stay as
 * permissive as it was: a row retrieved by the broad tier is still the
 * customer's property. Only RETRIEVAL is tiered.
 */
export function addressVariants(addr) {
  const { specific, broad } = addressVariantTiers(addr);
