...
export function stashVerdict(data, county) {
  try {
     ...
     *
     * It cannot happen through the two obvious failures: storage throwing takes
     * both writes down together, and both are called from the same click. It can
     * happen through a roll row whose situs is empty, which is a data condition
     * nobody controls. So the two writes share one precondition rather than
     * agreeing informally about when they apply.
     */
