...
export const OUTCOMES = {
  ...
  /**
   * TEXAS, IN A DISTRICT WE HAVE NOT LOADED. Added 7 Sept 2026.
   *
   * NOT `no_parcel`. We never asked a roll — we hold none for that county — so
   * "nothing like it came back" would be a claim about data we never queried.
   * Florida has one DOR roll covering all 67 counties, which is why this
   * outcome had no reason to exist until Texas: there, partial coverage is the
   * normal state for a long time (six districts of 254).
   *
   * `no_answer` rather than `our_failure`: nothing is broken. The fix is a
   * business one — load the district — and the funnel counts it separately so
   * we can see which counties are being asked for. That count is the roadmap.
   */
  not_covered:            { group: 'no_answer', label: 'We hold no roll for that district yet' },
