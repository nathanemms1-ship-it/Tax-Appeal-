...
import hearingOutcomes from './tx/hearingOutcomes';

/**
 * ============================================================================
 * OUR OWN MEASUREMENT OF A DISTRICT'S PUBLISHED RECORD — 10 Sept 2026
 * ============================================================================
 * Everything else in this file is a figure somebody else computed and published,
 * typed in by hand and ageing from the day it was typed. The Harris entries
 * below are different: they are computed by scripts/tx/hearing-outcomes.mjs from
 * arb_hearings_real.txt, the per-account hearing file HCAD publishes and nobody
 * else in Texas does. Re-running the generator updates the site.
 *
 * They are still THIRD-PARTY OUTCOMES for a jurisdiction, not our results — the
 * framing rules above apply unchanged, and OUTCOME_DISCLAIMER must accompany
 * them. TaxAppeal USA has still never filed a protest.
 *
 * THE DENOMINATOR IS THE WHOLE CLAIM. 79.1% is reductions divided by every A1
 * account with protested=Y on the roll — 424,251 of them. The 31,029 that filed
 * and never appear in the hearing file count as NO reduction. Measured against
 * accounts that reached a hearing instead, the figure is 85.2%, and that is the
 * number NOT published here: a homeowner reads "X% of protests win" as "if I
 * file, my odds are X", and the honest denominator for that sentence is everyone
 * who filed.
 *
 * Percentages round DOWN. 79.1 is published as 79%.
 */
const HARRIS = hearingOutcomes?.districts?.['101'] || null;
...
export const STATS = {
  ...
  /**
   * The headline. Any reduction in appraised value, denominator = everyone who
   * filed. Null when the generated file is absent, so callers must fall back
   * rather than render "undefined%".
   */
  TX_HARRIS_PROTEST_WIN_RATE: HARRIS ? {
    value: `${Math.floor(HARRIS.reducedPctOfFiled)}%`,
    label: `of the ${HARRIS.filed.toLocaleString()} single-family protests filed in Harris County received a lower appraised value`,
    source: 'Harris Central Appraisal District, ARB hearing file (arb_hearings_real.txt)',
  ...
  /**
   * A reduction in APPRAISED VALUE, not a tax saving. Never render this next to
   * the $89 fee in a way that reads as a return on it — framing rule 4 above.
   */
  TX_HARRIS_MEDIAN_REDUCTION: HARRIS ? {
    value: `$${HARRIS.medianReduction.toLocaleString()}`,
    label: 'median reduction in appraised value where a Harris County protest succeeded',
    source: 'Harris Central Appraisal District, ARB hearing file (arb_hearings_real.txt)',
    url: 'https://hcad.org/pdata/pdata-property-downloads.html',
    year: HARRIS.taxYear,
    confidence: 'high',
  ...
  /**
   * The one a homeowner deciding whether to hire a contingency firm actually
   * wants. Both denominators are accounts that reached a hearing, because the
   * roll does not record WHO filed for the 31,029 with no hearing record — so
   * this is a like-for-like comparison on the only basis the data supports, and
   * it is not the same denominator as TX_HARRIS_PROTEST_WIN_RATE above.
   */
  TX_HARRIS_OWNER_VS_AGENT: HARRIS ? {
    value: `${HARRIS.ownerFiled.reducedPct}% vs ${HARRIS.agentFiled.reducedPct}%`,
    label: 'Harris County reduction rate, owner-filed protests versus agent-filed',
    source: 'Harris Central Appraisal District, ARB hearing file (arb_hearings_real.txt)',
    url: 'https://hcad.org/pdata/pdata-property-downloads.html',
    year: HARRIS.taxYear,
    confidence: 'high',
