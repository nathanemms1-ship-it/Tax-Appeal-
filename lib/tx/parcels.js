 ...
 * is null.
 *
 * Omitting them does not error. It ages the subject on a different basis from
 * the candidates -- which comps selects for itself, with both columns present
 * -- and silently disables the condition penalty for every subject in Texas.
 * A quietly worse comp set, on the one number the whole filing rests on.
 * Added 7 Sept 2026 by tracing the seam instead of trusting it.
 */
const COLS = [
