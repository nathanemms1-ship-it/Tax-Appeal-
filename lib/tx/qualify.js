 ...
 * the IAAO Standard on Ratio Studies metric the Comptroller's Property Value
 * Study also applies:
 *
 *     cad  county      median   p75     COD   sellable
 ...
 *
 * Every one of these five sits INSIDE the IAAO residential standard of COD ≤ 15,
 * so none of them is doing bad work. Kaufman is simply doing very tight work,
 * almost certainly straight off a cost schedule, and 1.4% is what is left after
 * competence. That is a market to price differently or skip, not a bug.
 *
...
export const REDUCTION_BY_CAD = {
  123: { plausible: 0.093, optimistic: 0.195, cod: 9.9 },
  129: { plausible: 0.014, optimistic: 0.036, cod: 1.6 },
  178: { plausible: 0.059, optimistic: 0.123, cod: 6.5 },
  221: { plausible: 0.079, optimistic: 0.171, cod: 10.1 },
  243: { plausible: 0.067, optimistic: 0.130, cod: 8.0 },
};
...
export function reductionExpectation(cadId) {
  ...
  return hit
    ? { ...hit, measured: true }
    : { plausible: PLAUSIBLE_REDUCTION_PCT, optimistic: OPTIMISTIC_REDUCTION_PCT, cod: null, measured: false };
}
