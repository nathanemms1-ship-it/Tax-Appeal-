...
export const BELOW_AVERAGE_CONDITION = Object.freeze({
  /**
   * DALLAS (57) — the first district to publish condition at all.
   *
   * DCAD's RES_DETAIL carries CDU_RATING_DESC (Condition / Desirability /
   * Utility). Observed across 628,449 residential accounts on the 2026
   * certified roll, 9 Sept 2026:
   *
   *   GOOD                          229,550
   *   AVERAGE                       148,250
   *   VERY GOOD                     140,640
   *   EXCELLENT                      43,953
   *   UNASSIGNED                     32,029
   *   FAIR                           22,903   <- below average
   *   MANUALLY ENTER DEPRECIATION     6,910
   *   POOR                            3,027   <- below average
   *   VERY POOR                         844   <- below average
   *   UNDESIRABLE                       437   <- below average
   *
   * 24,432 loaded parcels (4.3%) fall in the four below-average ratings.
   *
   * UNASSIGNED and MANUALLY ENTER DEPRECIATION are deliberately NOT here.
   * Neither is a condition finding: the first is an absent rating, and the
   * second records that the appraiser overrode the depreciation table, which
   * says nothing about the house. Counting them would fire a double-count
   * disclosure at 38,939 owners the district never rated poorly — the same
   * class of error as the guessed vocabulary this comment block warns about.
   *
   * scripts/tx/load-dcad.mjs writes these verbatim and uppercased. Keep the two
   * lists identical: the loader decides what reaches condition_code, this
   * decides what it means, and a divergence is silent.
   */
  57: new Set(['FAIR', 'POOR', 'VERY POOR', 'UNDESIRABLE']),

  // 71 El Paso, 220 Tarrant: their exports carry NO condition field at all —
  // PACS 8.0.x does not have one. Absent, not unknown. See conditionDiscountRisk.
});


/**
 * ============================================================================
 * ✅ RE-BASED 7 Sept 2026 — this block is kept as the record of why.
 * ============================================================================
 *
 ...
 *
 * DONE: CVV_REGIONS and CVV_REGIONAL_BASE in lib/costToCure.js carry the West
 * South Central figures, and txCostToCure passes that region. Alabama is East
 * South Central and still has none, so an AL packet still flags.
 *
