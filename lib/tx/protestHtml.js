...
// No business name, address or contact appears anywhere on the filed document:
// Nathan's call, 7 Sept 2026. The owner files pro se and the packet must read
// that way to the district. Anything the owner needs to know about who prepared
// it belongs in the Terms and the confirmation email, not on the protest.
// Asserted in verify-tx-protest.mjs — do not reintroduce a name or an address here.
...
/**
 * Escape a value that may be a string OR an array of sentences.
 *
 * comps.adjustments and comps.disclosure are ARRAYS (describeAdjustments and
 * disclosureFor both return string[]). Interpolating one straight into a
 * template literal runs it through String(), which comma-joins with no space:
 * "...drawn from the district's code.,All comparables share...". The preview
 * script never showed this because its fixture passed a single hand-written
 * string, so the renderer was only ever exercised on the wrong shape. Anything
 * reaching this from comps.js must go through here.
 */
export function lines(v) {
  if (v === null || v === undefined) return [];
  return (Array.isArray(v) ? v : [v]).map(String).filter((t) => t.trim() !== '').map(e);
}

/** Array-or-string rendered as one flowing paragraph of sentences. */
export function sentences(v) {
  return lines(v).join(' ');
}
...
export function renderForm(p) {
  ...
  <div class="sec"><div class="sh">SECTION 1 — PROPERTY OWNER</div><div class="sb">
    ...
    <div class="note"><b>The appraisal district will write to you at this address.</b>
      Any hearing notice, settlement offer or decision comes directly to you, and to no one else.</div>
  </div></div>
  ...
  <div class="sec"><div class="sh">SECTION 4 — YOUR OPINION OF VALUE</div><div class="sb">
    <div class="row"><div class="f">
      ...
      <div class="fv"><b>${f.opinionOfValue ? money(f.opinionOfValue) : ''}</b></div>
    </div></div>
    <div style="font-size:8.5pt;color:#333;margin-top:4px">
      ${!f.opinionOfValue
        ? `This box is left blank on purpose. Under Tax Code § 41.44(d) a notice of protest is
           sufficient without an opinion of value — it needs only to identify you, identify the
           property and show that you disagree — so the protest is complete and every ground is
           preserved. Write a figure here yourself before you sign if you want to state one.`
        : p.conditionExhibit && n(p.conditionExhibit.cureDollars) > 0
        ? `Supported by the comparable-property analysis and the property-condition
           exhibit on the following pages. The comparables establish the value of a
           home of this type in this neighborhood; the condition exhibit states the
           cost of the defects that set this property below that standard.`
  ...
  <div class="sec"><div class="sh">SECTION 5 — HEARING</div><div class="sb">
    ...
    <div class="cbr"><div class="cb"></div><div>By telephone conference — <i>requires a sworn
      affidavit of evidence, submitted before the hearing begins.</i></div></div>
    <div class="cbr"><div class="cb"></div><div>By videoconference — <i>requires a sworn
      affidavit of evidence.</i></div></div>
    <div class="cbr"><div class="cb"></div><div>On written affidavit only — <i>requires a sworn
      affidavit of evidence.</i></div></div>
    ...
    <div class="note"><b>This choice is yours, and leaving it blank does not affect your filing.</b>
      ...
      least 10 days before the hearing date. The three remote options each require a sworn
      affidavit of evidence that you would prepare and submit yourself. Under Tax Code § 41.45(i)
      it must be attested before an officer authorized to administer oaths — in practice a notary —
      and under § 41.45(b) it must reach the board before the hearing begins. You are
      <b>not</b> required to use the comptroller’s Form 50-283 (§ 41.45(k)); it is simply the
      standard form for it.
      No one else is authorized to appear for you, so if you do not attend and do not submit an
      affidavit, no one will be there on your behalf.
      <br><br>
  ...
  <div class="sec"><div class="sh">SECTION 6 — CERTIFICATION</div><div class="sb">
    <div style="font-size:9pt;margin-bottom:10px">I am the owner of the property described above,
      or the owner’s authorized employee or affiliated entity. The information in this notice is
    ...
    <div class="note"><b>This form is not filed until you sign it.</b> You are filing on your own
      behalf. No Form 50-162 appointment of agent has been filed for this protest and none will be,
      so no one other than you is authorized to act on it.</div>
  </div></div>
</div>`;
}
...
export function renderGrid(p) {
  ...
  ${p.requestedValue ? `<div class="ask">
    <div>
  ...
  </div>` : `<div class="ask"><div>
      <div style="font-size:8.5pt;text-transform:uppercase;letter-spacing:.5px;color:#444">
        Value requested</div>
      <div style="font-size:8.5pt;color:#333">No figure is stated on this filing. The median
        above is the comparison; the opinion of value in Section 4 is the owner’s to give.</div>
    </div></div>`}

  ${lines(g.disclosure).length ? `<div class="note"><b>Before you sign, read this.</b>
    ${lines(g.disclosure).map((t) => `<div style="margin-top:4px">${t}</div>`).join('')}</div>` : ''}

  <div class="foot">
    <b>How these comparables were selected.</b>
    ${sentences(g.adjustments)}
    ${g.neighborhoodCode && g.level === 'neighborhood'
      ? `That code is ${e(g.neighborhoodCode)} — the district’s own grouping, not a radius chosen by us.`
      : ''}
    <br><br>
    ...
    for tax year ${e(p.taxYear)}. This exhibit was compiled at the property owner’s request from
    that published roll. It is submitted by the owner, who signed the notice of protest.
  </div>
</div>`;
}
...
export function renderConditionExhibit(p) {
  ...
  <div class="note" style="margin-top:14px"><b>What this exhibit is, and is not.</b>
    These figures are the cost of the repairs. They are <b>not</b> a claim that this
    property’s value falls by that same amount — what a repair costs and what a buyer
    deducts for it are different numbers, and no one has appraised this property. They
    are put before the board as evidence that this property is not in the ordinary
    condition the comparable properties are appraised in, and therefore belongs at or
    below the value requested on the notice of protest. What weight to give them is the
    board’s to decide.</div>

  <div class="foot">
    Compiled at the property owner’s request from information the owner supplied about the
    property’s condition. <b>No inspection of the property was performed by anyone in preparing this
    exhibit</b> — the figures are published cost estimates, not an inspector’s or appraiser’s
    findings. It is submitted by the owner, who signed the notice of protest.
  </div>
</div>`;
}
...
export function renderProtestHtml(packet) {
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8">'
    ...
    + STY + '</head><body>' + renderForm(packet)
    + (packet.hasGrid ? renderGrid(packet) : '')
