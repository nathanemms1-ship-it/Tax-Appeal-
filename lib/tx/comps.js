 ...
 */
// The ONLY import in this file, and lib/tx/rollYear.js has none of its own — see
// the header there for why comps.js must stay runnable without a database.
import { ROLL_YEAR } from './rollYear';
...
/**
 * ============================================================================
 * A PLACEHOLDER CLASS CODE IS NOT A CLASS. Added 7 Sept 2026.
 * ============================================================================
 *
 * PACS writes `*` into imprv_det_class_cd where no class is assigned. It appears
 * in the improvement detail rows for things that have no class of their own
 * (PATIO, BREEZEWAY) and it lands in our quality_class for the main area too.
 *
 * Measured on El Paso, tax year 2026, A1 parcels:
 *
 *     R3    171,338   75.1%   $142.69/sqft    -1.0% vs district
 *     R4     24,580   10.8%   $168.67        +17.0%
 *     *      21,779    9.5%   $139.32         -3.4%   <- 1 in 10 parcels
 *     R2      6,912    3.0%    $98.53        -31.7%
 *     R5      1,700    0.7%   $210.19        +45.8%
 *     R1      1,177    0.5%   $111.21        -22.9%
 *
 * R1-R6 is a real ladder — a 2.5x spread in $/sqft — so this field carries
 * genuine signal and the penalty below is worth having. But `*` is not a rung on
 * it. Its median sits within 3.4% of the district, i.e. right where the
 * unclassified would sit: it means "not recorded", not "this quality".
 *
 * `'*'` is a truthy string, so before this the penalty fired at full strength
 * between an `*` subject and an R3 comp — the same 0.5 as a genuine R2-vs-R5
 * mismatch, which is half a size-band of distance. That systematically demoted
 * correctly-classed comps for 21,779 subjects, and demoted those 21,779 as
 * candidates for everyone else. It never errored and it never showed up in a
 * count; it just quietly produced worse comp sets for one parcel in ten.
 *
 * Unknown must behave like null here: skip the term rather than assert a
 * difference that is not known to exist.
 */
export const PLACEHOLDER_CODES = Object.freeze(new Set(['*', '**', '-', '--', 'N/A', 'NA', 'NONE', 'UNK']));

export function codeOrNull(v) {
  if (v === null || v === undefined) return null;
  const t = String(v).trim().toUpperCase();
  return t === '' || PLACEHOLDER_CODES.has(t) ? null : t;
}
...
export function similarity(subject, comp, bands) {
  ...
  // codeOrNull collapses PACS placeholders to null so an unrecorded class is
  // treated as unknown, not as a class that differs. See the block above.
  const sQ = codeOrNull(subject.quality_class);
  const cQ = codeOrNull(comp.quality_class);
  const q = sQ && cQ ? (sQ === cQ ? 0 : 0.5) : 0;

  // condition_code is null for every Texas parcel we hold -- PACS export layout
  // 8.0.34 does not publish it -- so this term is inert today. Kept because the
  // moment a district does publish it, it should count.
  const sC = codeOrNull(subject.condition_code);
  const cC = codeOrNull(comp.condition_code);
  const c = sC && cC ? (sC === cC ? 0 : 0.25) : 0;
...
export function selectComps(subject, candidates, bands) {
  ...
  // Returns whatever it found, INCLUDING a set below MIN_COMPS. It used to
  // return null there, which conflated two different jobs — see MIN_COMPS.
 ...
 */
export async function findComps(subject, { rollYear = ROLL_YEAR, db, fetchFn } = {}) {
  ...
  const attempts = [];
  // The fullest short set seen anywhere on the ladder. Only used if no stratum
  // and band combination ever reaches MIN_COMPS.
  let best = null;
  ...
  for (const stratum of [...STRATA, COUNTY_TIER]) {
    ...
    for (const bands of bandsFor(stratum.level)) {
      ...
      /**
       * MIN_COMPS NOW GOVERNS THE SEARCH, NOT THE ANSWER.
       *
       * It used to do both jobs: it decided when the ladder could stop widening,
       * AND it decided whether we would file at all. Nathan's call, 7 Sept 2026 —
       * the second job was never ours. § 41.43(b)(3) says "a reasonable number of
       * comparable properties" and fixes no figure; there is no statutory floor.
       *
       * So a short set no longer ends the search — the ladder keeps loosening and
       * widening, looking for a better one, and only settles for the short set if
       * nothing fuller exists anywhere on the ladder. Keeping the best-so-far is
       * what makes that safe: we never trade a tight 4 for a sprawling 5.
       */
      if (comps.length < MIN_COMPS) {
        if (!best || comps.length > best.comps.length) {
          best = { stratum, bands, comps, candidates, truncated };
        }
        continue;
      }
  ...
  /**
   * Nothing reached MIN_COMPS. Return the fullest set we did find, evaluated
   * exactly like a full one, with `sufficient: false` saying so.
   *
   * The old version returned no comps at all here, and buildProtest turned that
   * into a refusal. Both halves were wrong: the district's own roll DID have
   * comparable properties, we simply had fewer than our own convention wanted,
   * and whether that is worth filing was never our decision to make.
   */
  if (best) {
    const result = evaluateSet(subject, best.comps);
    return {
      sufficient: false,
      reason: 'few_comparables',
      level: best.stratum.level,
      levelStrength: best.stratum.strength,
      bands: best.bands,
      poolSize: best.candidates.length,
      poolTruncated: best.truncated,
      comps: best.comps,
      attempts,
      ...result,
      // Never 'high' on a short set, whatever the stratum says.
      confidence: 'low',
      adjustments: describeAdjustments(best.stratum, best.bands, best.comps.length),
      disclosure: [
        `This comparison rests on ${best.comps.length} comparable ${best.comps.length === 1 ? 'property' : 'properties'} rather than the ${MIN_COMPS} we normally require. § 41.43(b)(3) asks for "a reasonable number of comparable properties" and sets no figure, but the appraisal district may argue that ${best.comps.length} is not a reasonable number, and a median drawn from so few moves a long way if any one of them is unusual.`,
        ...disclosureFor(best.stratum, best.bands, result, best.comps.length),
      ],
    };
  }
  ...
  return {
    ...
    attempts,
    comps: [],
    message: `The appraisal district's roll has no comparable properties we can match to this one — nothing shares its neighborhood grouping, size and age closely enough to compare. There is no unequal-appraisal exhibit to build from it.`,
  };
}
...
export function describeAdjustments(stratum, bands, count) {
  const where = {
    neighborhood: "the appraisal district's own neighborhood code for this property",
    subdivision: 'the same recorded subdivision',
    neighborhood_group: "the appraisal district's neighborhood group",
...
export function disclosureFor(stratum, bands, result, count) {
  ...
  if (stratum.strength === 'weak' || stratum.strength === 'last_resort') {
    out.push(`There were not enough similar properties in your immediate neighborhood to build a comparison, so we widened to ${stratum.level === 'county_class' ? 'similar properties across the county' : "the district's broader market area"}. The appraisal district may argue these are not close enough comparisons.`);
  }
