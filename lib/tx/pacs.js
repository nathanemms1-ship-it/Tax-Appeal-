...
const LIVING_INCLUDE = /MAIN AREA|LIVING AREA|LIVING QUARTERS|\bDWELLING\b|\bRESIDENCE\b|MOBILE HOME|MANUFACTURED HOME|SECOND STORY|SECOND FLOOR|UPPER STORY|UPPER FLOOR|\bHOUSE\b|\bMAIN\b/i;

/** Fallback for the rare district that ships a blank description. */
export const LIVING_AREA_CODES = new Set(['MA', 'MA2', 'MA3', 'LV', 'LA', 'HSE', 'SEC', 'MH', 'LQ']);
...
export function isLivingArea(typeCode, description) {
  ...
  if (d) {
    if (LIVING_EXCLUDE.test(d)) return false;
    if (LIVING_INCLUDE.test(d)) return true;
    return false;
  }
  return LIVING_AREA_CODES.has((typeCode || '').trim().toUpperCase());
}
