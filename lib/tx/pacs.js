...
export const APPRAISAL_INFO = {
  recordLength: 9714,
  /**
   * 8.0.33 RECORDS ARE 9,922 BYTES, NOT 9,714 — AND EVERY FIELD BELOW IS STILL
   * AT THE SAME OFFSET.
   *
   * Tarrant's export is layout 8.0.33 and every one of its 1,404,499 rows was
   * counted "ragged" against the 8.0.34 length. It parsed correctly anyway,
   * because the two layouts are byte-identical up to 9,372 and 8.0.33 only
   * carves names out of regions 8.0.34 leaves as `filler` (ownership_pct,
   * mineral_lease_id) before running 208 bytes longer.
   *
   * A ragged counter that reports 100% on a healthy file is worse than none:
   * the next district with genuinely misaligned records reports 100% too, and
   * nobody looks. Hence a set of known-good lengths rather than one number.
   */
  recordLengths: new Set([9714, 9922]),
  /** Owner's share of the property, 8.0.33 only. See OWNER ROWS in load.mjs. */
  ownership_pct:      F(4022, 4027),
  prop_id:            F(1, 12),
 ...
 */
/**
 * UNCONDITIONED SPACE IS NOT LIVING AREA, WHATEVER ELSE THE DESCRIPTION SAYS.
 * Added 7 Sept 2026 so that ADDITION (below) cannot sweep in El Paso's
 * M- "ADDITION (NO HEAT OR AIR)", which the district itself values at $62.45/sqft
 * against $97.37 for MAIN AREA. Exclude is tested first, so this wins.
 * Only "NO HEAT" is observed in a real file; UNHEATED and UNFINISHED are the same
 * concept in other wording and exclusion is the safe direction.
 */
// 'ENCLOSED ROOM' is Tarrant's, and it has to be excluded BY NAME because the
// bare word ENCLOSED is an INCLUDE below: El Paso's ME "ENCLOSED" is 101% of
// main area and genuinely is living space, while Tarrant's "Enclosed Room" is
// 40% and is not. Same word, opposite meaning, so the two-word phrase is the
// only thing that separates them. This was counting 1,109 Tarrant segments as
// living area before the district's own $/sqft was checked.
//
// 'OUTDOOR' is Jefferson's, added 20 Sept 2026 for the same reason. Its
// OLA "OUTDOOR LIVING AREA" (733 segments, avg 155 sqft) matches the
// 'LIVING AREA' include below word for word and has been counted as living
// space since that pattern was written. A covered outdoor slab is not a room.
// Nothing legitimate is lost: the only other OUTDOOR descriptions in any
// export held are Kaufman's OUTDOOR KITCHEN and OUTDOOR FIREPLACE.
//
// 'DET. BONUS' is El Paso's, and it is the SECOND case of the same word meaning
// opposite things in two districts. Denton's BR "Bonus Room" is $128.40/sqft,
// 109% of its own main area, and is living space. El Paso's BR "DET. BONUS ROOM"
// is $28.10/sqft, 28% of its own main area, and is a casita in the yard. The
// phrase 'BONUS ROO' below would have swept 1,217 El Paso segments across 1,117
// properties into living area, 92 of which currently have no living area at all,
// so this exclude is what makes that include safe.
// Only the detached form is excluded. El Paso abbreviates it 'DET.'; every other
// export held spells 'DETACHED' out and uses it for garages, carports and
// storage, which the GARAGE / CARPORT / STORAGE excludes already catch. Denton's
// DL "Detached Living" ($95.47, 81% of main) is deliberately NOT caught: the
// word 'detached' is not the discriminator, the district's own rate is.
const LIVING_EXCLUDE = /DET\..*BONUS|DETACHED BONUS|\bOUTDOOR\b|ENCLOSED ROOM|NO HEAT|UNHEATED|UNFINISHED|GARAGE|CARPORT|\bPORCH\b|PATIO|STORAGE|UTILITY|OUT ?BUILDING|OUTBLDG|CANOPY|\bDECK\b|BALCONY|CONCRETE|ASPHALT|PAVING|\bPAV|POOL|SHED|BARN|FENCE|COMMERCIAL|OFFICE|WAREHOUSE|GREENHOUSE|CARPT|AWNING|\bSLAB\b/i;
...
const LIVING_INCLUDE = new RegExp([
  ...
  'LIVING QUAR',
  // ADDITIONS AND ENCLOSED ROOMS ARE LIVING SPACE, AND THE DISTRICT SAYS SO.
  // Added 7 Sept 2026 from imprv_det_val / imprv_det_area across five districts.
  // Value per square foot is the district's own statement of what a segment is:
  //
  //   El Paso   MA  MAIN AREA               $97.37   <- the reference
  //             ME  ENCLOSED                $98.77   101% of main  -> living
  //             M+  ADDITION                $92.55    95% of main  -> living
  //             ME- ENCLOSED                $70.73    73%          -> living
  //             M-  ADDITION (NO HEAT/AIR)  $62.45   blocked above -> not
  //             G   GARAGE                  $55.73    57%          -> not
  //             O   OPEN PORCH              $25.04    26%          -> not
  //   Kaufman   LA  LIVING AREA             $99.43
  //             ADNA Addition, Average      $76.14    77% of main  -> living
  //
  // 62,385 El Paso properties (~21%) carry one, averaging 579 sqft against a
  // ~1,900 sqft main area. Every one of them ALSO has a main-area segment, so
  // this was never a zeroing bug — it was a 30% size understatement on a fifth
  // of the county, which biases the subject and the comps in opposite directions.
  //
  // SAFE ACROSS DISTRICTS BECAUSE EXCLUDE IS TESTED FIRST. Jefferson's
  // ENP "ENCLOSED PORCH/PATIO" ($18.73) and Wichita's PEP "ENCLOSED PORCH"
  // ($35.51) are genuinely not living space, and both are caught by the existing
  // PORCH / PATIO excludes before these patterns are ever reached. Verified
  // against Nueces, Jefferson, Kaufman, Taylor and Wichita exports, not just the
  // district that prompted the change.
  'ADDITION', 'ENCLOSED',
  // DENTON, added 20 Sept 2026. Same measurement, the district's own numbers
  // (imprv_det_val / imprv_det_area, segments >400 sqft):
  //
  //   MA  Main Area                 $117.91   <- the reference, avg 1,902 sqft
  //   BR  Bonus Room                $128.40   109% of main  -> living
  //   MA2 Second Floor              $115.77    98%          -> living already
  //   BH  Bath House / Pool House   $109.70    93%          -> not, see below
  //   DL  Detached Living            $95.47    81%          -> living
  //   AA  Attached Addition          $80.28    68%          -> living already
  //   EG  Enclosed Garage            $64.36    55%          -> not
  //   AG  Attached Garage            $51.57    44%          -> not
  //
  // BR is 48,355 segments on a 381,106-row export and NOTHING caught it: the
  // code is not in LIVING_AREA_CODES and 'Bonus Room' matches no pattern here.
  // Denton values it higher per square foot than its own main area.
  //
  // Matched on the stem 'BONUS ROO' for the char(25) truncation reason given
  // above. Wichita's AGBR "ATTACHED GARAGE/BONUS ROO" is the one description
  // in any export where that stem is NOT living area ($21.50, 6 segments), and
  // GARAGE excludes it before this line is reached.
  'BONUS ROO',
  // '\bLIVING\b' catches DL "Detached Living", which 'LIVING AREA' and
  // 'LIVING QUAR' both miss. Leaving it out understates living area, which
  // inflates the subject's $/sqft and biases towards filing a weak case — the
  // direction the Tarrant block below warns about. Every LIVING description in
  // the seven exports held was measured before widening this: the only one that
  // is not living space is Jefferson's OUTDOOR LIVING AREA, now excluded above.
  '\\bLIVING\\b',
 ...
 *
 * A CODE HERE MATCHES IN EVERY DISTRICT, INCLUDING ONES WHOSE EXPORT WE HAVE
 * NEVER SEEN, so only codes that can mean nothing else belong here. Denton's
 * BR is a bonus room valued above its own main area; Nueces's BR is a BOAT
 * RAMP. Same two letters, and only the description separates them, which is
 * why BR is matched as 'BONUS ROO' in LIVING_INCLUDE above and is deliberately
 * absent from this set. scripts/verify-tx-pacs.mjs asserts it stays absent.
 */
export const LIVING_AREA_CODES = new Set([
  'MA', 'MA2', 'MA3', 'LV', 'LVS', 'LA', 'HSE', 'SEC', 'MH', 'LQ', 'DWL', 'RES',

  // TARRANT, added 9 Sept 2026. Its dwelling code is RESMAIN, and NOTHING above
  // caught it — 99.1% of 1.4M parcels loaded with no living area at all. Note
  // why the description rule missed it too: LIVING_INCLUDE carries '\bMAIN\b',
  // but "ResMain" has no word boundary before "Main", so it never matched.
  //
  // The other two are the same call the El Paso block below documents, made on
  // Tarrant's own numbers (imprv_det_val / imprv_det_area, segments >200 sqft):
  //
  //   ResMain     ResMain            $86.23   <- the reference, avg 1,406 sqft
  //   ResAddn     ResAddn            $92.19   107% of main  -> living
  //   ResConvGar  ResConvGar         $76.19    88%          -> living
  //   ResS        Studio             $63.38    73%          -> LEFT OUT, see below
  //   ResER       Enclosed Room      $34.41    40%          -> not
  //   ResEP       Enclosed Porch     $30.28    35%          -> not (PORCH excluded)
  //   ResDG       Detached Garage    $32.41    38%          -> not
  //   ResG        Garage             $32.03    37%          -> not
  //
  // ResS "Studio" (4,376 segments) is deliberately NOT here. At 73% it sits
  // exactly where El Paso's ME- ENCLOSED was included, but a "studio" may be a
  // detached art studio rather than living quarters and the name does not say.
  // Leaving it out UNDERSTATES living area, which inflates the subject's $/sqft
  // and biases towards filing — the wrong direction. Worth resolving by checking
  // whether ResS ever appears on a parcel with no ResMain segment.
  //
  // DENTON'S BH "Bath House / Pool House" (580 segments) is the same open
  // question in another district: 93% of main area by Denton's own valuation,
  // but POOL excludes it and that exclude is load-bearing elsewhere (Wichita's
  // RCAB "CABANA - POOL HOUSE" is 74%). 580 segments on 381,106 rows, in the
  // understating direction, so it is logged rather than changed.
  'RESMAIN', 'RESADDN', 'RESCONVGAR',
]);
