 ...
 */
export function opinionOfValue(parcel, comps, cureDollars = 0) {
   ...
   *
   * equity  § 41.43(b)(3) — the median APPRAISED value of comparables. Condition
   *         is not part of that comparison and must not be subtracted from it.
   * market  § 41.41(a)(1) — what the comparables indicate this house is worth in
   *         average condition, LESS what it costs to cure its documented
   *         defects. This is where condition belongs.
   */
  const equity = n(comps.indicatedAppraised);
  const marketGround = n(comps.indicatedMarket) - n(cureDollars);

  /**
   * THE FLOOR IS THE DISTRICT'S OWN LAND VALUE.
   *
   * A large enough cure could otherwise drive the ask below the value of the
   * bare lot, which is not a proposition any board will entertain and which
   * makes the rest of the packet look unserious. Using the district's own
   * land_value rather than a number of ours keeps the floor arguable.
   */
  const floor = n(parcel.land_value);

  /**
   * The market ground is FLOORED, not filtered.
   *
   * `[equity, marketGround, market].filter(v => v > 0)` was the first version and
   * it had a hole: a cure larger than the indicated market value made that term
   * negative, the filter dropped it, and the whole condition case vanished
   * silently — the owner with the worst house got the same ask as one with no
   * defects at all. Caught by the floor assertion, which failed because the
   * floor was never reached.
   *
   * A negative market ground is not missing data. It means the repairs exceed
   * what the comparables say the house is worth, and the honest answer to that
   * is the land value, not the equity figure.
   */
  const candidates = [];
  if (equity > 0) candidates.push(equity);
  if (n(comps.indicatedMarket) > 0) candidates.push(marketGround);
...
export function buildProtest({ parcel, comps, owner = {}, taxYear, filingDate = null,
  ...
  const opinion = opinionOfValue(parcel, comps, cureDollars);
  ...
  return {
    ...
    conditionExhibit: (cure.priced.length || cure.narrative.length) ? {
      ...
      // ⚠️ True until the Cost vs. Value bases are re-derived from the West South
      // Central report. See the block comment in lib/tx/costToCure.js.
