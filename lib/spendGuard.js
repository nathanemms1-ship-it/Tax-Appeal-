...
export const DAILY_BUDGET = {
  ...
  // properties costs the same as one returning one. Foundation plan is $74/mo for
  // 1,000 calls with a $0.06 overage, so this ceiling is denominated in a number
  // we can reason about directly: 2,000 calls/day of overage is ~$120/day.
  //
  // Deliberately NOT sized for "we would never spend that". Sized so that a bad
  // day is a bill we would notice and survive, because the vendor-side control is
  // weaker here — RentCast bills overage automatically rather than cutting off.
  // Set a plan-level cap in the RentCast dashboard as well.
  rentcast: Number(process.env.DAILY_CAP_RENTCAST || 2000),
