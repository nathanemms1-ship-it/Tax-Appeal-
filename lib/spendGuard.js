...
/**
 * ============================================================================
 * HOW LONG WE WAIT FOR UPSTASH — raised from 1200 ms on 15 Sept 2026
 * ============================================================================
 * The monitor emailed "[WARN] Spend ceilings: cannot read usage — Redis
 * unreachable" nine lines above "OK Redis", on the same run, about the same
 * Upstash instance. Both checks run concurrently in the same lambda, so both pay
 * the same cold TLS handshake — but checkRedis() allows 6000 ms for a single
 * /ping while this module allowed 1200 ms for a multi-key PIPELINE. The tight
 * one lost the race and reported a cause the patient one disproved four minutes
 * later, when /api/health came back "ok" with nothing changed.
 *
 * 1200 ms never covered a cold connection. It is the enforcement path for a
 * KEYSTROKE endpoint though (/api/autocomplete calls checkSpend per miss), so
 * this cannot simply be set to six seconds — a customer would feel it.
 *
 * 2500 ms is the trade: comfortably above a cold Upstash round trip, still below
 * anything a person notices between keystrokes.
 *
 * DELIBERATELY SHARED WITH THE MONITOR. spendUsage() reads on the same budget
 * checkSpend() enforces on, because a monitor that waits LONGER than production
 * reports "ok" while real requests are timing out — it would under-report
 * exactly the condition it exists to catch.
 */
export const SPEND_TIMEOUT_MS = Number(process.env.SPEND_TIMEOUT_MS || 2500);
const spendSignal = () =>
  (typeof AbortSignal?.timeout === 'function' ? AbortSignal.timeout(SPEND_TIMEOUT_MS) : undefined);
...
export async function spendUsage() {
  if (!REST_URL || !REST_TOKEN) {
    lastReadError = 'UPSTASH_REDIS_REST_URL / _TOKEN are not set';
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      lastReadError = `Upstash pipeline returned HTTP ${res.status}`;
      return null;
    }
    const body = await res.json();
    lastReadError = null;
    ...
    /**
     * KEEP THE REASON. This used to `return null` for four different causes —
     * missing env vars, a non-2xx from Upstash, a timeout, or any other throw —
     * and lib/healthChecks.js, receiving only falsiness, asserted "Redis
     * unreachable" in an email whose very next check said Redis was reachable.
     *
     * A null still goes back so every existing caller behaves identically; the
     * cause is left here for the monitor to report instead of invent.
     */
    lastReadError = e?.name === 'TimeoutError' || /abort/i.test(e?.message || '')
      ? `timed out after ${SPEND_TIMEOUT_MS}ms`
      : (e?.message || String(e));
    return null;
  }
}

/** Why the last spendUsage() failed, for the monitor. Null when it succeeded. */
let lastReadError = null;
export function lastSpendReadError() { return lastReadError; }
