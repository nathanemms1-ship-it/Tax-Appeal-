...
/**
 * ============================================================================
 * COST vs. VALUE IS REGIONAL, AND THAT IS THE POINT OF IT
 * ============================================================================
 * The CVV entry below is tier 1 *because* it is regional rather than national.
 * It was pinned to South Atlantic — Florida's region — and used for Texas too,
 * so a Texas filing cited Florida contractor pricing to an El Paso ARB while
 * printing "South Atlantic region" on the page.
 *
 * The error ran the dangerous way. West South Central costs are 6-16% LOWER,
 * and overstating cost to cure overstates the reduction we ask for:
 *
 *     roof, asphalt shingle    32,253 SA  ->  27,116 WSC   -15.9%
 *     bath remodel, midrange   25,609     ->  22,995       -10.2%
 *     HVAC                     19,208     ->  17,241       -10.2%
 *     windows, vinyl           21,629     ->  20,171        -6.7%
 *     kitchen, minor midrange  28,567     ->  26,762        -6.3%
 *
 * Six of 33 priced defects are CVV-sourced, and they are the six most
 * expensive, so they dominate any realistic cure total.
 *
 * ⚠️ EAST SOUTH CENTRAL (Alabama) IS NOT ENTERED. Only regions whose figures
 * have actually been read are listed. An unmapped state keeps South Atlantic
 * and is flagged by lib/tx/costToCure.js sourceRegionMismatch, which is the
 * honest state: a wrong number is worse than a known-missing one.
 */
export const CVV_REGIONS = Object.freeze({
  'south-atlantic': {
    label: 'JLC/Zonda Cost vs. Value Report, South Atlantic region',
    url: 'https://www.jlconline.com/cost-vs-value/2025/south-atlantic/',
  },
  'west-south-central': {
    label: 'JLC/Zonda Cost vs. Value Report, West South Central region',
    url: 'https://www.jlconline.com/cost-vs-value/2025/west-south-central/',
  },
});

/** Census divisions, which is how Cost vs. Value groups states. */
export const STATE_TO_CVV_REGION = Object.freeze({
  FL: 'south-atlantic', GA: 'south-atlantic',
  TX: 'west-south-central', AR: 'west-south-central',
  // AL is East South Central. Deliberately absent — see the warning above.
});

/**
 * Per-region job costs, read from that region's report. A label absent here
 * falls back to its `base`, which is South Atlantic.
 */
export const CVV_REGIONAL_BASE = Object.freeze({
  'west-south-central': Object.freeze({
    'Roof damage or age (leaks, missing shingles, sagging)': 27116,
    'Outdated or failed HVAC system': 17241,
    'Broken windows, doors, or security issues': 20171,
    'Severely dated interiors requiring major renovation': 26762,
    'Severely dated kitchen requiring full update': 26762,
    'Severely dated bathrooms requiring full update': 22995,
  }),
});
 ...
 */
/**
 * `region` selects the Cost vs. Value division. Defaults to South Atlantic, so
 * every existing Florida caller — and scripts/verify-*.mjs — behaves exactly as
 * before. lib/tx/costToCure.js passes 'west-south-central'.
 */
export function curePriceFor(issueLabel, parcel, { position = CURE_POSITION, region = 'south-atlantic' } = {}) {
  ...
  /**
   * The regional base replaces spec.base BEFORE the multipliers, not after —
   * size and finish scale a job cost, so applying them to the wrong region's
   * cost and correcting later would compound the error rather than remove it.
   */
  const regionalBase = spec.source === 'CVV'
    ? (CVV_REGIONAL_BASE[region]?.[issueLabel] ?? spec.base)
    : spec.base;

  const mid = Math.round((regionalBase * size * finish) / 100) * 100;

  /**
   * And the CITATION follows the number. Printing a West South Central figure
   * under a South Atlantic label would be the one thing worse than the wrong
   * number: an accurate figure with a false provenance, on a document the owner
   * signs and defends.
   */
  const baseSrc = SOURCES[spec.source];
  const src = (spec.source === 'CVV' && CVV_REGIONS[region])
    ? { ...baseSrc, label: CVV_REGIONS[region].label, url: CVV_REGIONS[region].url }
    : baseSrc;
 ...
 */
export function totalCostToCure(issues, parcel, overrides = {}, { position = CURE_POSITION, region = 'south-atlantic' } = {}) {
  const priced = [];
  const narrative = [];
  let total = 0;
  ...
  for (const issue of issues || []) {
    const p = curePriceFor(issue, parcel, { position, region });
    if (!p) continue;
