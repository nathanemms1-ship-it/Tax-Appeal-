...
/**
 * ============================================================================
 * DOES THE FIRST STEP OF THE FUNNEL REFUSE THIS ADDRESS OUTRIGHT?
 * ============================================================================
 * Lives here rather than inline in pages/apply.js because it is a decision, not
 * a render, and a decision inside a 4,600-line React page cannot be tested.
 *
 * The rule it replaces read:
 *
 *   if (countyName && ws && !ws.canFile && !ws.canPreOrder) { onClosedWindow(); }
 *
 * `countyName` is there for a real reason — 24 Aug 2026, a Census 403 on a
 * Broward address produced a null county, strict mode fell back to the EARLIEST
 * Florida deadline, and the homeowner hit a terminal "deadline is in 9 days"
 * screen that was false about their county. In Florida and Georgia the county
 * IS the date, so an unresolved county is not grounds for refusal.
 *
 * But pages/apply.js only resolves a county for GA and FL. For Texas countyName
 * is ALWAYS null, so the condition could never be true and no Texas address was
 * ever gated — found 10 Sept 2026 by typing a Mansfield address into the live
 * funnel, out of season, and landing on the property-condition step.
 *
 * Texas has ONE STATEWIDE WINDOW. getFilingWindowStatus('TX', null) returns the
 * same answer as 'TX','Tarrant' and as 'TX','Nosuchcounty'. A null county there
 * is not a missing fact, so it is not a reason to wave anyone through.
 */
export function windowBlocksEntry(stateCode, countyName, ws) {
  if (!ws) return false;
  // Selling is live, or pre-orders are. Not a refusal either way.
  if (ws.canFile || ws.canPreOrder) return false;

  /**
   * WINDOW OPEN, BUT TOO CLOSE TO THE DEADLINE TO PROMISE DELIVERY.
   *
   * This is the ONE case where the county still matters, and it is the 24 Aug
   * defect: in Florida and Georgia the county IS the date, strict mode falls
   * back to the earliest deadline in the state, and a Census 403 on a Broward
   * address produced a terminal "your deadline is in 9 days" that was false
   * about their county. An unresolved county is not grounds for that refusal.
   *
   * Texas has one statewide deadline, so there is nothing left to establish.
   */
  if (ws.isOpen && ws.tooClose) {
    return String(stateCode || '').trim().toUpperCase() === 'TX' ? true : !!countyName;
  }

  /**
   * OFF SEASON. Everyone, every state, no county required.
   *
   * The next season's opening and pre-order dates are STATE-level facts —
   * getFilingWindowStatus('TX', null) returns the same answer as 'TX','Tarrant'
   * and as 'TX','Nosuchcounty'. So there is no fact left to establish and no
   * reason to wave anyone deeper into a funnel that cannot sell them anything.
   */
  return true;
}
