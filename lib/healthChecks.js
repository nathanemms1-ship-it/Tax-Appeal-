...
import { spendUsage, DAILY_BUDGET, lastSpendReadError } from './spendGuard';
...
export async function checkSpendCeilings() {
  ...
  if (!usage) {
    /**
     * SAY WHAT HAPPENED, NOT WHAT WE ASSUME — 15 Sept 2026.
     *
     * This read: "cannot read usage — Redis unreachable, so the ceilings are not
     * counting". It went out in an email nine lines above "OK Redis", about the
     * same Upstash instance, on the same run. spendUsage() returned a bare null
     * for any of four causes and this line picked one and stated it as fact.
     *
     * The real cause was a 1200 ms timeout on a cold connection while checkRedis
     * allowed 6000 ms for a lighter request — now a single shared budget, see
     * SPEND_TIMEOUT_MS. The reason is carried out of spendGuard rather than
     * guessed at here.
     */
    const why = lastSpendReadError() || 'reason not recorded';
    return result('Spend ceilings', 'warn',
      `cannot read usage (${why}) — the ceilings are not counting while this persists.\n`
      + 'Check the Redis line below: if it says OK, Upstash is reachable and this is\n'
      + 'latency against SPEND_TIMEOUT_MS, not an outage.');
  }
 ...
 *
 * ============================================================================
 * RETRIES ONCE IN-RUN, FOR THE REASON checkDatabase DOES. 25 Aug.
 * ============================================================================
 * At 15:00:27Z the monitor sent `[WARN] Stuck orders — check failed: timeout after
 * 6000ms`. In the SAME report Database, Schema, Filing deadlines, Lead capture,
 * Visitor counter and Check outcomes — every other check that queries Supabase —
 * returned OK. Supabase was not degraded; had it been, Database would have been
 * critical in the same email.
 *
 * This is the identical failure the DB_PROBE_ATTEMPTS header describes for 24 Aug,
 * arriving at the next-most-exposed probe. That fix was applied only to
 * checkDatabase, so the contention simply moved.
 *
 * And this check is the MOST exposed of the set, for a structural reason: it is one
 * of fifteen fired through a single Promise.all, and it alone issues TWO requests of
 * its own. It has double the chance of holding the connection that stalls, so of all
 * the Supabase probes it was always going to be the next one to lose the race.
 *
 * The trade is the same one already argued at DB_PROBE_ATTEMPTS and not re-argued
 * here: retry inside the same invocation, no persisted state, a real outage fails
 * both attempts and still alerts on this very run, and a probe that fails then
 * succeeds names the failed attempt in its detail so a flap stays visible.
 */
export async function checkStuckOrders() {
  ...
  const sig = () => (typeof AbortSignal?.timeout === 'function' ? AbortSignal.timeout(TIMEOUT_MS) : undefined);

  const failures = [];
  for (let attempt = 1; attempt <= DB_PROBE_ATTEMPTS; attempt++) {
    try {
      const [nr, aw] = await Promise.all([
        withTimeout(fetch(`${url}/rest/v1/orders?select=id,created_at&dispute_status=eq.needs_review`, { headers, signal: sig() })),
        withTimeout(fetch(`${url}/rest/v1/orders?select=id,created_at&dispute_status=eq.awaiting_signature&created_at=lt.${cutoff}`, { headers, signal: sig() })),
      ]);

      // Same rule as checkDatabase: a non-2xx is a failed ATTEMPT, not a verdict.
      // A 5xx from PostgREST is the same transient shape as a stall; a 401/404 will
      // fail twice and report identically to before.
      if (!nr.ok || !aw.ok) throw new Error(`query failed (${nr.status}/${aw.status})`);
      ...
      if (failures.length) parts.push(`(read on attempt ${attempt}; earlier attempts failed: ${failures.join('; ')})`);
      ...
      return result(
        ...
        'ok',
        failures.length
          ? `no paid order is stuck (read on attempt ${attempt}; earlier attempts failed: ${failures.join('; ')})`
          : 'no paid order is stuck',
        { needsReview: 0, awaiting: 0 }
      );
    } catch (e) {
  ...
  /*
    EXHAUSTED. Deliberately NOT 'ok'.

    This is the one check where a false all-clear is the most expensive thing the
    monitor could say — a customer who paid and whose petition never went out has
    no recourse once the statutory deadline passes. So an unreadable orders table
    stays a warn.

    But the wording has to say WHICH kind of warn it is. The old text was
    "check failed: timeout after 6000ms", which at a glance in an inbox reads as a
    finding about orders rather than a statement about the probe — and this check's
    real firing is CRITICAL, "PAID BUT NOT MAILED". A warn that looks like that one
    spends the attention that one needs.
  */
  return result(
    'Stuck orders',
    'warn',
    `could not read the orders table — ${DB_PROBE_ATTEMPTS} attempt(s) failed (${failures.join('; ')}). `
      + 'This says nothing about whether an order is stuck: that is UNKNOWN, not clear. '
      + 'If the Database check in this same report is OK, the likely cause is request '
      + 'contention inside runAllChecks rather than anything wrong with the orders table.',
    { unknown: true }
  );
}
