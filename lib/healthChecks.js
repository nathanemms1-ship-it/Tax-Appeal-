...
const REQUIRED_ENV = [
  ...
  { key: 'UPSTASH_REDIS_REST_URL', sev: 'warn', why: 'rate limiting and every cache are disabled; the spend ceilings stop counting' },
  { key: 'UPSTASH_REDIS_REST_TOKEN', sev: 'warn', why: 'rate limiting and every cache are disabled; the spend ceilings stop counting' },
  { key: 'GOOGLE_PLACES_API_KEY', sev: 'warn', why: 'address autocomplete returns nothing; customers must type addresses by hand' },
  { key: 'BATCHDATA_API_KEY', sev: 'warn', why: 'no property data; every customer is pushed to manual entry' },
];

export function checkEnv() {
  const missing = REQUIRED_ENV.filter((e) => {
  ...
  const lines = missing.map((m) => `  ${m.sev === 'critical' ? '[CRITICAL]' : '[warn]'} ${m.key} — ${m.why}`);
  return result(
    'Configuration',
