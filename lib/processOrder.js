  ...
  try {
    ...
    const dr486Res = await fetch(`${BASE}/api/generate-dr486`, {
      ...
      body: JSON.stringify({
        ...
        county: order.county,
        // orders has no parcel_id column — the parcel/folio lives in account_number.
        // `order.parcel_id` was always undefined, so this silently relied on the roll
        // refresh alone. Unified on account_number 5 Aug 2026.
        parcelId: fresh.parcelId || order.account_number || '',
        assessedValue,
