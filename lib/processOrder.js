...
export async function dispatchQueuedOrder(order, { supabase, resend }) {
  ...
  /**
   * ======================================================================
   * FLORIDA: NO PARCEL AND NO ASSESSED VALUE MEANS NO MAILING.
   * ======================================================================
   * Added 11 Aug 2026. Last line of defence, behind the funnel gate in
   * pages/apply.js StepFloridaCheck and the refusal in pages/api/generate-dr486.js.
   *
   * `missingCore` above required exactly two fields — a letter body and a street.
   * Neither says anything about WHICH parcel the petition disputes or what value it
   * disputes, so an order with `account_number` and `assessed_value` both null
   * passed this preflight, had a real cheque cut against it, and was mailed.
   *
   * Worth being concrete about what the county then receives. The mailroom splits
   * the piece: the cheque goes to finance, the petition to the VAB clerk. With no
   * parcel the cheque memo falls back to the owner's surname (buildCheckMemo in
   * pages/api/send-letter.js) and the petition's own folio box is empty — so there
   * is no key on either side of that split. Under § 194.013 the fee is a condition
   * of filing and Florida is satisfied by RECEIPT, so an unmatched fee is a
   * petition quietly held, with no notice to us and no recovery after the deadline.
   *
   * Scoped to Florida deliberately. Texas and Georgia mail a letter to an appraisal
   * district rather than a fee-bearing statutory petition keyed on a folio number,
   * and their windows are shut — widening this to them without first checking what
   * their forms actually require would refuse orders we can complete.
   *
   * The message names the remedy because by the time this fires the customer has
   * already paid: this should be unreachable for anything bought after today, so an
   * order tripping it is money held for something that cannot be delivered.
   * cron/process-queued-orders.js alerts on the first failure and writes the reason
   * to the row, so it surfaces in /admin instead of a log.
   */
  if (isFLOrder) {
    const flMissing = [];
    if (!order.account_number || !String(order.account_number).trim()) flMissing.push('parcel/folio number');
    if (!(Number(order.assessed_value) > 0)) flMissing.push('assessed value');
    if (flMissing.length) {
      return {
        success: false,
        error: `FL order is missing ${flMissing.join(' and ')} — the petition cannot identify the parcel or state the value in dispute. REFUND THIS ORDER; it cannot be filed.`,
      };
    }
  }
