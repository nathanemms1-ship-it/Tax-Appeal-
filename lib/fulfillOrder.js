  ...
  try {
    await fetch(`${BASE_URL}/api/send-email`, {
      ...
      body: JSON.stringify({
        ...
        orderStatus: status,
        // The signing page, so the receipt has something to click. /success reads
        // session_id from the query and /api/verify-payment authenticates on it —
        // this is the same URL Stripe already redirected the customer to, so putting
        // it in their own receipt grants nothing they did not already hold.
        signingUrl: `${BASE_URL}/success?session_id=${encodeURIComponent(session.id)}`,
